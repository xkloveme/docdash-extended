 ...
 */
var Signal = Base.compose(
    [Base, signals.Signal],
    /** @lends module:mixins/signalable~Signal# */ {
         ...
         */
        toString: function () {
            return format('{0}\nname:{1}\nlisteners:{2}', this.declaredClass, this.name, this.getNumListeners());
        }
    }
);
...
var Signalable = Base.compose(
    ...
    /** @lends mixins/signalable# */ {
        ...
        _addSignals: function (signals) {
            signals = signals || {};
            if (this.options) {
                signals = sys.extend({}, sys.result(this, 'signals'), signals);
            }
